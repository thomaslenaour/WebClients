...
import { HistoryFlag, OptimisticHistoryItem, OptimisticState } from '../types';
...
export const commitReducer = <T>(
    ...
    if (optimisticActionIndex === -1) {
        console.error(`Commit failure : No initiated optimistic action could be found for id ${optimisticId}.`);
        return optimistic;
    }

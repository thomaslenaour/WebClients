...
import { OptimisticFailedHistoryItem, OptimisticHistoryItem, OptimisticState } from '../types';
...
export const failReducer = <T>(optimistic: OptimisticState<T>, optimisticId: string): OptimisticState<T> => {
    ...
    if (historyItemIndex === -1) {
        console.error(`Fail failure : No initiated optimistic action could be found for id ${optimisticId}.`);
        return optimistic;
    }
